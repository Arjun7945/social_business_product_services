...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'joinedAt' | 'lastInteractionAt'> & {
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      joinedAt: customer.joinedAt?.toJSON() ?? null,
      lastInteractionAt: customer.lastInteractionAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      joinedAt: restCustomer.joinedAt ? dayjs(restCustomer.joinedAt) : undefined,
      lastInteractionAt: restCustomer.lastInteractionAt ? dayjs(restCustomer.lastInteractionAt) : undefined,
    };
  }
