  ...
  return (
    <div>
      <h2 id="order-item-heading" data-cy="OrderItemHeading">
        <Translate contentKey="whatsappProductServiceProApp.orderItem.home.title">Order Items</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="whatsappProductServiceProApp.orderItem.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order-item/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="whatsappProductServiceProApp.orderItem.home.createLabel">Create new Order Item</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderItemList && orderItemList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="whatsappProductServiceProApp.orderItem.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('quantityKg')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('quantityKg')} />
                </th>
                <th className="hand" onClick={sort('priceAtOrder')}>
                  <Translate contentKey="whatsappProductServiceProApp.orderItem.priceAtOrder">Price At Order</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('priceAtOrder')} />
                </th>
                <th>
                  <Translate contentKey="whatsappProductServiceProApp.orderItem.product">Product</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="whatsappProductServiceProApp.orderItem.order">Order</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {orderItemList.map((orderItem, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{orderItem.quantityKg}</td>
                  <td>{orderItem.priceAtOrder}</td>
                  <td>{orderItem.product ? <Link to={`/fish-product/${orderItem.product.id}`}>{orderItem.product.name}</Link> : ''}</td>
                  <td>{orderItem.order ? <Link to={`/customer-order/${orderItem.order.id}`}>{orderItem.order.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="whatsappProductServiceProApp.orderItem.home.notFound">No Order Items found</Translate>
            </div>
          )
        )}
      </div>
