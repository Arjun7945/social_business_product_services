  ...
  return (
    <div>
      <h2 id="shopping-cart-heading" data-cy="ShoppingCartHeading">
        <Translate contentKey="whatsappProductServiceProApp.shoppingCart.home.title">Shopping Carts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="whatsappProductServiceProApp.shoppingCart.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/shopping-cart/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="whatsappProductServiceProApp.shoppingCart.home.createLabel">Create new Shopping Cart</Translate>
          </Link>
        </div>
      </h2>
        ...
        {shoppingCartList && shoppingCartList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="whatsappProductServiceProApp.shoppingCart.id">ID</Translate>{' '}
            ...
            <tbody>
              {shoppingCartList.map((shoppingCart, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {shoppingCart.createdAt ? <TextFormat type="date" value={shoppingCart.createdAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {shoppingCart.updatedAt ? <TextFormat type="date" value={shoppingCart.updatedAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {shoppingCart.customer ? <Link to={`/customer/${shoppingCart.customer.id}`}>{shoppingCart.customer.name}</Link> : ''}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/shopping-cart/${shoppingCart.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="whatsappProductServiceProApp.shoppingCart.home.notFound">No Shopping Carts found</Translate>
            </div>
          )
        )}
      </div>
