 ...
 */
@Repository
public interface ReturnedOrderRepository extends JpaRepository<ReturnedOrder, Long>, JpaSpecificationExecutor<ReturnedOrder> {
    ...
    @Query(
        value = "select returnedOrder from ReturnedOrder returnedOrder left join fetch returnedOrder.customer",
        countQuery = "select count(returnedOrder) from ReturnedOrder returnedOrder"
    )
