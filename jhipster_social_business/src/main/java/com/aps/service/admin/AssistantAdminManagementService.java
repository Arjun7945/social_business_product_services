...
public class AssistantAdminManagementService {
    ...
    public AssistantAdminManagementService(
    ...
    public void showAssistantAdminMenu(TeamMember admin) {
        ...
        List<WhatsAppMessageDto.ButtonDto> buttons = List.of(
            ...
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
                .reply(WhatsAppMessageDto.ReplyDto.builder().id("SHOW_ALL_ASSISTANT").title("📋 Show All").build())
                .build(),
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
    ...
    public void handleAssistantAdminWaPhoneInput(TeamMember admin, BotSession session, String waPhone) {
        ...
        List<WhatsAppMessageDto.ButtonDto> buttons = List.of(
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
                .reply(WhatsAppMessageDto.ReplyDto.builder().id("ASSISTANT_ACTIVE_YES").title("Yes").build())
                .build(),
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
                .reply(WhatsAppMessageDto.ReplyDto.builder().id("ASSISTANT_ACTIVE_NO").title("No").build())
                .build()
        );

        whatsAppService.sendCartActionButtons(
            admin.getWaPhoneNumber(),
            "✅ WhatsApp: " + waPhone.trim() + "\n\n🔄 Is this assistant admin active?",
            buttons
        );
    ...
    public void finalizeAssistantAdminAdd(TeamMember admin, BotSession session) {
        ...
        try {
            ...
            e.printStackTrace();
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "❌ Error adding assistant admin: " + e.getMessage());
            showAssistantAdminMenu(admin);
        }
    }

    public void showAllAssistantAdmins(TeamMember admin) {
        List<TeamMember> assistantAdmins = teamMemberRepository
            .findAll()
            .stream()
        ...
        if (assistantAdmins.isEmpty()) {
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "📋 *No assistant admins found.*");
            showAssistantAdminMenu(admin);
            return;
        }

        StringBuilder message = new StringBuilder(String.format("📋 *All Assistant Admins* (Total: %d)\n\n", assistantAdmins.size()));
        int count = 1;
    ...
    public void startUpdateAssistantAdmin(TeamMember admin, BotSession session) {
        whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "🚧 *Update Assistant Admin*\n\nThis feature is coming soon!");
        showAssistantAdminMenu(admin);
    }

    public void startDeleteAssistantAdmin(TeamMember admin, BotSession session) {
        whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "🚧 *Delete Assistant Admin*\n\nThis feature is coming soon!");
        showAssistantAdminMenu(admin);
    }
}
