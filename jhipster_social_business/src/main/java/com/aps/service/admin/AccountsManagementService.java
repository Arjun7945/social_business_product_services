...
public class AccountsManagementService {
    ...
    public void showAccountsMenu(TeamMember admin) {
        ...
        List<WhatsAppMessageDto.ButtonDto> buttons = List.of(
            ...
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
                .reply(WhatsAppMessageDto.ReplyDto.builder().id("SHOW_ALL_ACCOUNTS").title("📋 Show All").build())
                .build(),
            WhatsAppMessageDto.ButtonDto.builder()
                .type("reply")
    ...
    public void startAddAccountsMember(TeamMember admin, BotSession session) {
        ...
        whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "➕ *Add New Accounts Member*\n\n📝 Please provide the member's name:");
    ...
    public void showAllAccountsMembers(TeamMember admin) {
        List<TeamMember> members = teamMemberRepository.findAll().stream().filter(tm -> tm.getRole() == UserRole.ACCOUNTS_TEAM).toList();
        ...
        StringBuilder message = new StringBuilder(String.format("📋 *All Accounts Team Members* (Total: %d)\n\n", members.size()));
        int count = 1;
