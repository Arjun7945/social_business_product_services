...
public class CreditCustomerFlowService {
        ...
        List<WhatsAppMessageDto.ButtonDto> buttons = List.of(
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder()
                                .id(PREFIX_ADMIN_CREDIT_ALLOW + orderId + "_" + deliveryPerson.getId())
                                .title(deliveryPersonMessageService.getButtonAllowCreditOnce()).build())
                        .build(),
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder()
                                .id(PREFIX_ADMIN_CREDIT_GRANT + orderId + "_" + deliveryPerson.getId())
                                .title(deliveryPersonMessageService.getButtonGrantAlways()).build())
                        .build(),
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder()
                                .id(PREFIX_ADMIN_CREDIT_DENY + orderId + "_" + deliveryPerson.getId())
                                .title(deliveryPersonMessageService.getButtonDenyCredit()).build())
                        .build());
    ...
    public void handleAdminDecision(TeamMember admin, String buttonId) {
        ...
        if (order == null || dp == null) {
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "❌ Error: Order or Delivery Person not found.");
            return;
        }
        ...
        if (prefix.equals(PREFIX_ADMIN_CREDIT_GRANT)) {
            ...
            whatsAppService.sendSimpleText(dp.getWaPhoneNumber(),
                    deliveryPersonMessageService.getCreditGrantedAlwaysMessage(customer.getName(), orderId));
            ...
            try {
                whatsAppService.sendSimpleText(customer.getWaPhoneNumber(),
                        customerMessageService.getMessageCreditPrivilegesGranted(customer.getName()));
            } catch (Exception e) {
            ...
            // Notify DP
            whatsAppService.sendSimpleText(dp.getWaPhoneNumber(),
                    deliveryPersonMessageService.getCreditApprovedOnceMessage(customer.getName(), orderId));

            // Notify Admin
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "✅ Approved: Only for this Order" + orderId);

        } else {
            // DENY
            // Notify DP
            whatsAppService.sendSimpleText(dp.getWaPhoneNumber(),
                    deliveryPersonMessageService.getCreditDeniedMessage(customer.getName(), orderId));

            // Notify Admin
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "❌ Denied Credit for Order " + orderId);
        }
    }
    ...
    public void showCreditCustomerMenu(TeamMember admin) {
        List<WhatsAppMessageDto.RowDto> rows = List.of(
                ...
                WhatsAppMessageDto.RowDto.builder()
                        .id(MENU_CREDIT_CRUD)
                        .title("👥 CRUD Credit Cust")
                        .description("Show Credit Customers").build(),
                WhatsAppMessageDto.RowDto.builder()
                        .id("BACK_TO_MAIN")
                        .title("⬅️ Back")
    ...
    public void showCreditCrudMenu(TeamMember admin) {
        List<WhatsAppMessageDto.RowDto> rows = List.of(
                WhatsAppMessageDto.RowDto.builder()
                ...
                WhatsAppMessageDto.RowDto.builder()
                        .id("CREDIT_CUSTOMER_SECTION")
                        .title("⬅️ Back")
                        .description("Credit Menu").build());
        whatsAppService.sendInteractiveList(admin.getWaPhoneNumber(), "👥 *Credit Customer CRUD*", rows);
    }
    ...
    public void showCreditCustomerOrders(TeamMember admin) {
        ...
        if (orders.isEmpty()) {
            whatsAppService.sendSimpleText(admin.getWaPhoneNumber(), "No credit customer orders found till now.");
            return;
        }
        ...
        rows.add(WhatsAppMessageDto.RowDto.builder().id("CREDIT_MENU").title("⬅️ Back").description("Credit Menu")
                .build());
    ...
    public void handleCreditOrderSelection(TeamMember admin, Long orderId) {
        ...
        String details = String.format(
                "📦 *Order Details*\n" +
                        ...
                        "Amount: ₹%.2f\n" +
                        "Status: %s\n",
                order.getId(), order.getCustomer().getName(), order.getTotalAmount(), order.getStatus());

        List<WhatsAppMessageDto.ButtonDto> buttons = List.of(
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder().id(PREFIX_CREDIT_PAY_LINK + orderId)
                                .title("🔗 Send Link").build())
                        .build(),
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder().id(PREFIX_CREDIT_COD + orderId)
                                .title("💵 Marked COD").build())
                        .build(),
                WhatsAppMessageDto.ButtonDto.builder().type("reply")
                        .reply(WhatsAppMessageDto.ReplyDto.builder().id(MENU_CREDIT_OC).title("⬅️ Back").build())
                        .build());
    ...
    public void handleCreditOrderAction(TeamMember admin, String buttonId) {
        if (buttonId.startsWith(PREFIX_CREDIT_PAY_LINK)) {
            ...
            if (order != null) {
                ...
                String link = razorpayService.createPaymentLink(order.getId(), order.getTotalAmount().doubleValue(),
                        order.getCustomer());
                if (link == null) {
                    whatsAppService.sendSimpleText(admin.getWaPhoneNumber(),
                ...
                whatsAppService.sendSimpleText(order.getCustomer().getWaPhoneNumber(),
                        String.format("Hi %s, payment link for order %d from %s: %s",
                                order.getCustomer().getName(), orderId, admin.getName(), link));
            }
